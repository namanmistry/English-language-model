...
class EnglishDataCleaning:
    ...
    def clean_fast(self, batch_size, start_batch_no=0):
        ...
        if config["Last_batch_cleaned"] <= len(os.listdir(self.data_dir)):
            ...
            list_of_files = os.listdir(self.data_dir)[
                start_batch_no:start_batch_no+batch_size]
            print("List of files in cleaning is: ",list_of_files)
    ...
    def fast_cleaning(self, batch_no, data):
        ...
        try:
            ...
            with open(f'{self.save_dir}/{batch_no}', 'wb') as f:
...
class DataPreprocessing:
    '''This class takes takes cleaned english text and preprocesses it and converts it into the sequences.
        Which can further be used for training the model.'''
    ...
    def sort_files(self,list_of_files):
        t = list(map(lambda x: int(x.split('_')[-1]),list_of_files))
        t_1 = sorted(t)
        t_2 = list(map(lambda x: str(x),t_1))
        return t_2
    ...
    def fast_make_save_sequences(self):
        with open('preprocessed.json', 'r') as f:
            config = json.load(f)
        list_of_files = self.sort_files(os.listdir(self.data_dir))
        ...
        print("list of files in sequencing: ",list_of_files[config['last_batch_preprocessed']:config['last_batch_preprocessed']+self.batch_size])
        self.fast_sequencing(
            list_of_files[config['last_batch_preprocessed']:config['last_batch_preprocessed']+self.batch_size])
    ...
    def fast_sequencing(self, list_of_files):
        ...
        try:
            ...
            for i in (done_files):
                with open(self.save_dir + f'{i[0]}', 'wb') as f:
                    pickle.dump(i[1], f)
            print("number of samples: ",len(done_files[0][1]))
            done_files[:] = []
        except Exception as e:
            print(e)
...
class CustomPreprocessor:
    '''This class takes the sequences and converts them into MLM dataset format which is necessary to train the model.'''
    ...
    def fast_make_save_MLM_dataset(self):
        ...
        if not os.path.isdir(self.save_dir):
            os.mkdir(self.save_dir)
        self.make_MLM_dataset(list_of_files[config['last_batch_custom_preprocessed']
                              :config['last_batch_custom_preprocessed']+self.batch_size])
    ...
    def make_MLM_dataset(self, list_of_files):
                        ...
                        if k < self.seq_len:
                            if random.random() <= self.mask_rate:
                                if len(t) < self.max_mask_per_seq:
                                    ...
                                    t_2.append(1)
                                    i[k] = self.mask_id
                    weights.append(t_2 if len(t_2) == self.max_mask_per_seq else (
                        t_2 + [0]*(self.max_mask_per_seq-len(t_2))))
                    mask_positions.append(t if len(t) == self.max_mask_per_seq else (
                        t+[0]*(self.max_mask_per_seq-len(t_2))))
                    target_values.append(t_1 if len(t_1) == self.max_mask_per_seq else (
                        t_1+[0]*(self.max_mask_per_seq-len(t_1))))
                    new_sequences.append(
                        i[:self.seq_len]+[0]*(self.seq_len-len(i[:self.seq_len])))
        ...
        try:
            ...
            for i in (done_files):
                print(f"Saving mlm batch {i[0]}")
                with open(self.save_dir + f'{i[0]}', 'wb') as f:
                    pickle.dump(i[1], f)
            print("number of samples: ",len(done_files[0][1][0]['tokens']))
            done_files[:] = []
        except Exception as e:
...
class LanguageModel():
    '''This class makes a BERT model and trains that model and after each epoch it loads the tensorboard which is used for 
        monitoring the word embeddings'''
    ...
    def make_bert(self):
        ...
        outputs = keras.layers.LayerNormalization(
            epsilon=self.norm_epsilon)(outputs)
    ...
    def fit(self, model, train_ds, epochs, batch_size):
        ...
        list_of_files = self.sort_files(os.listdir(train_ds))
        print("in training list of file sbefore: ",list_of_files,start_batch_no,start_batch_no+self.transfer_learning_batch)
        list_of_files = os.listdir(train_ds)[start_batch_no:start_batch_no+self.transfer_learning_batch]
        print("in training lisgt of files: ",list_of_files)
        ...
        if config['last_trained_batch'] <= len(os.listdir(train_ds)):
            ...
            training_data = ({"tokens": tf.convert_to_tensor(tf.concat([d[0]['tokens'] for d in data], 0)),
                                 ...
                                 tf.convert_to_tensor(
                                     tf.concat([d[1] for d in data], 0)),
                                 tf.convert_to_tensor(tf.concat([d[2] for d in data], 0)))
            if not os.path.exists(self.model_checkpoint + f'/{start_batch_no+self.transfer_learning_batch}'):
                os.mkdir(self.model_checkpoint + f'/{start_batch_no+self.transfer_learning_batch}')
            print("number of samples in training is: ",(training_data[0]['tokens']))
            check_point = ModelCheckpoint(
                self.model_checkpoint + f'/{start_batch_no}', monitor='sparse_categorical_accuracy', mode='max')
            ...
            model.save(self.models_save_path +
                        f'model_transfer_step_{start_batch_no+self.transfer_learning_batch}_with_epochs_{epochs}')

            device.reset()
            print(
                f"Training of batch {start_batch_no} done. now procedding to the next transfer learning step.\n")
...
class Main():
    '''This is the main class which handles all the operations going into the training of the model'''
        ...
        while True:
            ...
            if len(os.listdir(self.custom_preprocessing_save_dir)) != prev:
                ...
                if len(os.listdir(self.models_save_path)) != 0:
                    try:
                        model = load_model(
                            self.models_save_path + f'/{os.listdir(self.models_save_path)[-1]}')
                        language_model_handler = LanguageModel(seq_len=self.seq_len,
                                                               ...
                                                               transfer_learning_batch=self.transfer_learning_batch,
